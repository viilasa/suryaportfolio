...
import { useAllPosts, useCategories } from '../hooks/useBlog';
import { useState } from 'react';
import type { BlogPost } from '../types';

// Predefined categories — always shown in this order
...
const getTags = (keywords?: string): string[] =>
    keywords ? keywords.split(',').map(t => t.trim()).filter(Boolean).slice(0, 3) : [];
...
const HeroCard = ({ post }: { post: BlogPost }) => (
    <Link to={`/blogs/${post.slug}`} className="blogs-hero__main hover-lift">
        ...
        <div className="blogs-hero__content">
            ...
            {getTags(post.keywords).length > 0 && (
                <div className="blog-tags">
                    <Tag size={12} />
                    {getTags(post.keywords).map(tag => (
...
const SecondaryCard = ({ post, index }: BlogCardProps) => (
    <Link
        to={`/blogs/${post.slug}`}
    ...
    >
        ...
        <div className="blogs-hero__secondary-content">
            ...
            {post.excerpt && (
                <p className="blogs-hero__secondary-excerpt">{post.excerpt}</p>
            )}
            {getTags(post.keywords).length > 0 && (
...
const BlogsPage = () => {
    ...
    const categories = [
        ...
        ...dynamicCategories.filter(c => !PREDEFINED_CATEGORIES.includes(c)),
    ];

    // Filter by tag — a post can have multiple tags so check the tags array
                ...
                <div className="page__container">
                    ...
                    {gridPosts.length > 0 ? (
                        ...
                        <div className="blogs-empty">
                            <p>No posts in <strong>{activeCategory}</strong> yet — coming soon.</p>
                        </div>
                    )}
                </div>
            </main>
